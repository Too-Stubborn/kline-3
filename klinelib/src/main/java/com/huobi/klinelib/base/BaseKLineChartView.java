...
import com.huobi.klinelib.formatter.TimeFormatter;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint lineEndPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private float intervalLastAnim = 0.01f;
    private float lastPrice;

    private IAdapter<ICandle> mAdapter;
    ...
    private DataSetObserver mDataSetObserver = new DataSetObserver() {
        ...
        public void onChanged() {
            ...
            if (count > mItemCount) {
                lastPrice = getAdapter().getItem(count - 1).getClosePrice();
            }
            mItemCount = count;
            if (null != mainDraw) {
                mMainDraw.setItemCount(mItemCount);
            }
            notifyChanged();
        }
...
//            invalidate();
    ...
    private void drawK(Canvas canvas) {
        ...
        LogUtil.e("mTranslateX : " + mTranslateX);
        LogUtil.e("mScaleX : " + mScaleX);

        LogUtil.e(getMeasuredHeight() + "  : getMeasuredHeight");
        LogUtil.e(getMeasuredWidth() + "  : getMeasuredWidth");
        LogUtil.e(canvas.getWidth() + "  : getMeasuredWidth");
        LogUtil.e(canvas.getHeight() + "  : getMeasuredWidth");
        ...
        if (isLongPress) {
            ...
            // k线图竖线
            canvas.drawLine(x, mMainRect.top, x, mMainRect.bottom, selectedYLinePaint);
            // k线图横线
            canvas.drawLine(-mTranslateX, y, -mTranslateX + mWidth / mScaleX, y, selectedXLinePaint);
            // 柱状图竖线
            canvas.drawLine(x, mMainRect.bottom, x, mVolRect.bottom, selectedYLinePaint);
            if (mChildDraw != null) {
                // 子线图竖线
                canvas.drawLine(x, mVolRect.bottom, x, mChildRect.bottom, selectedYLinePaint);
            }
        }
        //还原 平移缩放
        canvas.restore();
...
//            canvas.drawText(formatValue(mMainMaxValue), mWidth - calculateWidth(formatValue(mMainMaxValue)), baseLine + mMainRect.top, mTextPaint);
            //设置横线时,数字 显示在线上方
            canvas.drawText(formatValue(mMainMaxValue), mWidth - calculateWidth(formatValue(mMainMaxValue)), mMainRect.top, mTextPaint);
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, mWidth - calculateWidth(text), fixTextY(rowSpace * i + mMainRect.top) - textHeight, mTextPaint);
            }
        }
        //--------------画中间子图的值-------------
        if (mVolDraw != null) {
        ...
        if (isLongPress) {
            ...
            float tempX = textWidth + 2 * w1;
            if (translateXtoX(getX(mSelectedIndex)) < getChartWidth() / 2) {
                x = 1;
                Path path = new Path();
                path.moveTo(x, y - r);
                path.lineTo(x, y + r);
                path.lineTo(tempX, y + r);
                path.lineTo(tempX + w2, y);
                path.lineTo(tempX, y - r);
                path.close();
                canvas.drawPath(path, selectPointPaint);
                canvas.drawPath(path, selectorFramePaint);
                canvas.drawText(text, x + w1, fixTextYBaseBottom(y), mTextPaint);
            } else {
                ...
                canvas.drawPath(path, selectPointPaint);
                canvas.drawPath(path, selectorFramePaint);
                canvas.drawText(text, x + w1 + w2, fixTextYBaseBottom(y), mTextPaint);
            }
    ...
    private void drawPriceLine(Canvas canvas) {
        ...
        IKLine item = (IKLine) mAdapter.getItem(mAdapter.getCount() - 1);
        float closePrice = item.getClosePrice();
        float y = getMainY(closePrice);

        float textWidth = priceLineBoxRightPaint.measureText(String.valueOf(closePrice));
        float x = (getX(mStopIndex - mStartIndex) + cancleWidth / 2) * getScaleX();

        float rightSpace = mWidth - x;

        if (rightSpace > textWidth) {
            Path path = new Path();
            for (float i = x; i < mWidth - textWidth; i += 12) {
                path.moveTo(i, y);
                path.lineTo(i + 8, y);
            }
            canvas.drawPath(path, priceLineBoxRightPaint);
            canvas.drawText(String.valueOf(closePrice), mWidth - textWidth, fixTextYBaseBottom(y), priceLineBoxRightPaint);

        } else {
            float halfPriceBoxHeight = priceLineBoxHeight / 2;
            //修改价格信息框Y轴计算保证,只会绘制在主区域中
            if (closePrice > mMainMaxValue) {
                y = mMainRect.top + halfPriceBoxHeight;
            } else if (closePrice < mMainMinValue) {
                y = mMainRect.bottom - halfPriceBoxHeight;
            }

            Path path = new Path();
            for (int i = 0; i < mWidth; i += 12) {
                path.moveTo(i, y);
                path.lineTo(i + 8, y);
            }
            //硬件加速会导致DashPathEffect失效,考虑到绘制比较多还是手动画虚线
//            DashPathEffect dashPathEffect = new DashPathEffect(new float[]{6, 4}, 0);
            ...
            priceLineBoxPaint.setColor(Color.BLACK);
            priceLineBoxPaint.setStyle(Paint.Style.FILL);
            Paint.FontMetrics fontMetrics = mTextPaint.getFontMetrics();

            float width = mTextPaint.measureText(String.valueOf(closePrice));
            float boxLeft = mWidth - width - priceBoxMarginRight - halfPriceBoxHeight;
            float boxTop = y - halfPriceBoxHeight;
            float boxRight = mWidth - priceBoxMarginRight + halfPriceBoxHeight;
            ...
            canvas.drawText(String.valueOf(closePrice), mWidth - width / 2 - priceBoxMarginRight, (y + (fontMetrics.descent - fontMetrics.ascent) / 2 - fontMetrics.descent), mTextPaint);
    ...
    public int getMinScrollX() {
        return (int) -(getmOverScrollRange());
    }
    ...
    public void drawLastMinutsLine(Canvas canvas, Paint paint, float startX, float startValue, float stopX, float stopValue) {
        canvas.drawLine(startX, getMainY(startValue), stopX, getMainY(lastPrice), paint);
    }

    public void drawLastMinutsLineArea(Canvas canvas, Paint paint, float startX, float startValue, float stopX, float stopValue) {
        Path path = new Path();
        path.moveTo(startX, displayHeight + mTopPadding + mBottomPadding);
        path.lineTo(startX, getMainY(startValue));
        path.lineTo(stopX, getMainY(lastPrice));
        path.lineTo(stopX, displayHeight + mTopPadding + mBottomPadding);
        path.close();
        canvas.drawPath(path, paint);
    }

    public void drawEndPoint(Canvas canvas, float stopX, float stopValue) {
//        canvas.drawCircle(startX, getMainY(startValue), stopX, getMainY(stopValue), lineEndPaint);
        RadialGradient radialGradient = new RadialGradient(stopX, getMainY(lastPrice), lineEndPointWidth * 4, lineEndPaint.getColor(), Color.TRANSPARENT, Shader.TileMode.CLAMP);
        lineEndPaint.setShader(radialGradient);
        canvas.drawCircle(stopX, getMainY(lastPrice), lineEndPointWidth * 4, lineEndPaint);

        if (lastPrice == stopValue) {
            return;
        }
        if (lastPrice < stopValue) {
            lastPrice += intervalLastAnim;
        } else if (lastPrice > lastPrice) {
            lastPrice -= intervalLastAnim;
        }
        LogUtil.e(stopX);
        handler.postDelayed(freshLine, 1000);
    }

    Runnable freshLine = new Runnable() {
        @Override
        public void run() {
            invalidate();
        }
    ...
    public void setAdapter(IAdapter adapter) {
        ...
        if (mAdapter != null) {
            ...
            if (mItemCount > 0) {
                lastPrice = getAdapter().getItem(mItemCount - 1).getClosePrice();
            }
            if (mMainDraw != null) {
                mMainDraw.setItemCount(mItemCount);
            }
    ...
    public float getmOverScrollRange() {
        return mOverScrollRange / mScaleX;
    }
