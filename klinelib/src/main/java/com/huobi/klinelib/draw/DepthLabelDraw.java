...
public class DepthLabelDraw {
    ...
    public DepthLabelDraw(int xLabelCount, int yLabelCount) {
        ...
        selectedLabelPaint.setTextSize(25);
     ...
     * 绘制选中的lable
     *
     * @param floats x,y ,xposition,ypositio
     */
    public void drawSelectedLables(Canvas canvas, float[] floats) {
        ...
        if (floats[2] < halfTextWidth + selectedBorderWitdh / 2) {
            floats[2] = halfTextWidth + selectedBorderWitdh / 2;
        }
        if (floats[2] > width - halfTextWidth - selectedBorderWitdh / 2) {
            floats[2] = width - halfTextWidth - selectedBorderWitdh / 2;
        }
        float left = floats[2] - halfTextWidth;
        float right = floats[2] + halfTextWidth;
        float top = height - labelHeight + selectedBorderWitdh / 2;
        float bottom = height - selectedBorderWitdh / 2;
        ...
        right = width - selectedBorderWitdh / 2;
        top = floats[3] - labelHeight / 2 + selectedBorderWitdh / 2;
        bottom = floats[3] + labelHeight / 2 - selectedBorderWitdh / 2;
        canvas.drawRect(new RectF(left, top, right, bottom), selectedBoxPaint);
        canvas.drawRect(new RectF(left, top, right, bottom), selectedBoxBorderPaint);
